    ...
    const getItemById = async () => {
      try {
        const results = await axiosInstance.get(
          ITEM_ROUTES.GET_ITEM_BY_ID(itemId),
        );
        setCurrentItem(results.data.item);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  return (
    <div className="min-h-screen w-full bg-stone-50 flex flex-col items-center px-4 py-10">
      ...
      <div className="w-full max-w-md bg-white border border-stone-200 rounded-2xl shadow-sm p-7 sm:p-9">
        {/* ── Skeleton — shown while fetching item ── */}
        {!currentItem && (
          <div className="space-y-5 animate-pulse">
            <div className="h-3 w-28 bg-stone-100 rounded-full" />
            <div className="h-7 w-44 bg-stone-100 rounded-xl" />
            <div className="h-4 w-56 bg-stone-100 rounded-full mb-8" />
        ...
        {/* ── Form — shown once item is loaded ── */}
        {currentItem && (
          <>
            {/* Header */}
            <div className="mb-8">
